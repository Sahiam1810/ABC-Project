...
class CursoDetail extends HTMLElement{
    connectedCallback(){
        this.innerHTML=`<div class="modal" id="m"><div class="box" id="box">
            <header><h3 id="t">Curso</h3><button class="btn outline" id="x">Cerrar</button></header>
            <div id="c"></div></div></div>`;
        this.querySelector('#x').onclick=()=>this.close();
    }
    open(codigo){
        const db=read(); const c=db.cursos.find(x=>x.codigo===codigo);
        if(!c) return; const doc=db.docentes.find(d=>d.codigo===c.docenteCodigo);
        const mods=db.modulos.filter(m=>m.cursoCodigo===c.codigo);
        const wrap=document.createElement('div');
        wrap.innerHTML=`<div class="grid">
        <div>
            <img src="${c.foto||'https://via.placeholder.com/640x220?text=Curso'}" style="width:100%;height:220px;object-fit:cover;border-radius:12px">
    ...
    showLecciones(mod,db){
        ...
        list.forEach(l=>{
            ...
            it.innerHTML=`<b>${l.titulo}</b> <span class="badge">${l.horas||1} h</span>
                <p style="margin:6px 0">${l.contenido||''}</p>
                ${media?`<ul>${media}</ul>`:''}`;
        box.append(it);
        });
